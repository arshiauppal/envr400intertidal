...
#arshia version --> error bar dont work 
percent_cover_SPES <- function(quad0.25m_SPES) {
  # Calculate percentage of total cover comprised of algae and invertebrates
  quad0.25m_SPES$Algae_percent <- quad0.25m_SPES$Adjusted_Algae_Cover * 1
  quad0.25m_SPES$Invertebrates_percent <- quad0.25m_SPES$Adjusted_Invert_Cover * 1
  
  # Aggregate data by year and site_TA
  data_agg <- aggregate(cbind(Algae_percent, Invertebrates_percent) ~ Year + Site_TA, data = quad0.25m_SPES, FUN = mean, na.rm = TRUE)
  
  # Calculate standard deviation for each group
  algae_sd <- aggregate(Algae_percent ~ Year + Site_TA, data = quad0.25m_SPES, FUN = sd, na.rm = TRUE)$Algae_percent
  invertebrates_sd <- aggregate(Invertebrates_percent ~ Year + Site_TA, data = quad0.25m_SPES, FUN = sd, na.rm = TRUE)$Invertebrates_percent
  
  # Merge standard deviation into aggregated data
  data_agg <- merge(data_agg, data.frame(Year = rep(unique(data_agg$Year), each = length(unique(data_agg$Site_TA))), 
                                         Site_TA = rep(unique(data_agg$Site_TA), length(unique(data_agg$Year))),
                                         Algae_percent_sd = rep(algae_sd, each = length(unique(data_agg$Site_TA))),
                                         Invertebrates_percent_sd = rep(invertebrates_sd, each = length(unique(data_agg$Site_TA)))),
                    by = c("Year", "Site_TA"))
  
  # Calculate transition point between algae and invertebrates
  data_agg$Transition <- with(data_agg, Algae_percent / (Algae_percent + Invertebrates_percent))
  data_agg$Transition <- ifelse(is.nan(data_agg$Transition), 0.5, data_agg$Transition)
  
  # Reshape data for plotting
  data_plot <- reshape2::melt(data_agg, id.vars = c("Year", "Site_TA", "Algae_percent_sd", "Invertebrates_percent_sd", "Transition"))
  
  # Create the multi-colored bar graph with error bars
  ggplot(data = data_plot, aes(x = Year, y = value, fill = variable)) +
    geom_bar(stat = "identity", position = "stack") +
    geom_errorbar(aes(ymin = value - 0.5 * (ifelse(variable == "Algae_percent", Algae_percent_sd, Invertebrates_percent_sd)),
                      ymax = value + 0.5 * (ifelse(variable == "Algae_percent", Algae_percent_sd, Invertebrates_percent_sd))),
                  width = 0.2, position = position_dodge(width = 0.9)) +
    labs(x = "Year", y = "Percentage of Total Cover", fill = "Cover Component") +
    facet_wrap(~ Site_TA) +
    theme_minimal()
}
# jess version doesn't work rn but has better error bars when it does 
...
#=================================================================================================================================
# Aggregate mean sea stars count by season and Site_TA for SPES data
# works now- arshia 
